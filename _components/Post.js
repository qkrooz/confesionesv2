    ...
    return (
        <TouchableWithoutFeedback onPress={toggleImage}>
            <View>
                {hide
                    ? null
                    : <View style={{ width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', position: 'absolute', zIndex: 20 }}></View>
                }
                {hide
                    ? null
                    :
                    <Animatable.Text animation="pulse" iterationCount={2} easing="ease-out" style={{ position: 'absolute', zIndex: 30, bottom: 20, right: 30 }}>
                        <Icon disabled={true} name="eye" color={'white'} size={40} />
                    </Animatable.Text>
                }
                <View style={styles.ImageContainer}>
                    <Image source={CatIMG} style={{ width: '100%', height: '100%' }} blurRadius={blur} />
                </View>
            </View>
        </TouchableWithoutFeedback>
    );
}
const Post = ({ number, hasImage }) => {
    ...
    return (
        <LinearGradient start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} colors={[newColours[0], newColours[1]]} style={styles.postContainer}>
            {/* Card Header */}
            <View style={styles.headerContainer}>
                {/* Post Number and TimeStamp */}
                <View style={styles.headerInfo}>
                    {/* Post Number */}
                    <Text style={{ fontWeight: 'bold', fontSize: 30, marginRight: 5, color: textColor }}>{number}</Text>
                    {/* Time Stamp */}
                ...
                {/* More */}
                <TouchableWithoutFeedback>
                    <Icon name="more-horizontal" size={24} color={textColor} style={{ padding: 10, marginRight: 5 }} />
                </TouchableWithoutFeedback>
            </View>
            {/* Card Header Ends */}
            {hasImage
            ...
            <View style={styles.controlsContainer}>
                <TouchableWithoutFeedback
                    ...
                    <View style={styles.control}>
                        <Icon style={{ marginRight: 5 }} size={20} color={textColor} name="message-square" />
                        <Text style={{ color: textColor, fontSize: 15 }}>50</Text>
                    </View>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback>
                    <View style={styles.control}>
                        <Icon style={{ marginRight: 5 }} size={20} color={textColor} name="thumbs-up" />
                        <Text style={{ color: textColor, fontSize: 15 }}>100</Text>
                    </View>
                </TouchableWithoutFeedback>
                <TouchableWithoutFeedback>
                    <View style={styles.control}>
                        <Icon style={{ marginRight: 5 }} size={20} color={textColor} name="thumbs-down" />
